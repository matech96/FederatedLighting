...
project_name = f"{model}{NC}c{E}e{max_rounds}r{n_clients_per_round}f-{server_opt}-{client_opt_strategy[0]}-{client_opt}-early"
...
for cos_until in range(4):
    config = TorchFederatedLearnerEMNISTConfig(
        ...
        CLIENT_OPT_STRATEGY_UNITL=cos_until,
